...
def get_ghg_stats(layer_fire, layer_land_cover, gaul, codes):
    try:
        log.info(gaul)
        log.info(codes)
        ...
        query_extent = "SELECT ST_AsGeoJSON(ST_Extent(geom)) FROM spatial.gaul" + gaul + "_3857 WHERE adm1_code IN (" + codes + ")"
        query_layer = "SELECT * FROM spatial.gaul" + gaul + "_3857 WHERE adm1_code IN (" + codes + ")"
        ...
        return Response(json.dumps(land_cover_ba), content_type='application/json; charset=utf-8')
